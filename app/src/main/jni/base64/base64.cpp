...
static unsigned int pos_of_char(const unsigned char chr) {
    ...
    // Stealth error handling without exceptions
#ifdef NDEBUG
    // Release mode: Return safe value and log quietly
        __android_log_print(ANDROID_LOG_WARN, "Base64",
                          "Invalid character: %c (0x%02x)", chr, chr);
        return 0;  // Returns position of 'A' as fallback
#else
    // Debug mode: Force crash with stack trace
    __android_log_assert("Invalid base64 character",
                         "Base64",
                         "Invalid character: %c (0x%02x)", chr, chr);
    return 0; // Never reached, but satisfies return requirement
#endif
}
